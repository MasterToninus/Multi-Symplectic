...
Rigourosly, we define $\forall \phi \in \Conf$, $\forall \Sigma \subset M$ $m-1$ dimensional, compact submanifold

\begin{definition}[(covariant) pre-symplectic form in $\phi$ pertaining to $\Sigma$]
	\begin{displaymath}
		\Omega_\Gamma ( \phi ) : \textrm{T}_\phi \Conf \times \textrm{T}_\phi \Conf \rightarrow \Real
\end{displaymath}		
	S.t:
	\begin{align*}
		\left\langle	\Omega_\Sigma ( \varphi) \right.\left\lvert \delta \varphi_1 , \delta \varphi_2 \right\rangle &=		
		\int_\Gamma (j^1 \varphi)^\ast \left[ i_{\delta\varphi_2} i_{\delta \varphi_1} \omega_\Lagrangian \right]
		& \qquad \textrm{(Lagrangian framework)} \\
		\left\langle	\Omega_\Sigma ( \varphi, \pi) \right.\left\lvert ( \delta \varphi_1, \delta \pi_1),
		( \delta \varphi_2, \delta \pi_2) \right\rangle &=		
		\int_\Gamma (\varphi, \pi)^\ast \left[ i_{( \delta \varphi_2, \delta \pi_2)} i_{( \delta \varphi_1, \delta \pi_1)} \omega_\Hamiltonian \right]
		& \qquad \textrm{(Hamiltonian framework)} 		
	\end{align*}
\end{definition}

We can easily recognize that at this level $J^1 E$ and $\LinDualJet$ takes a similar role
